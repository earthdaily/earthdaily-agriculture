"""
Tests for HistoricalScoreExtractor processing pipeline:
    - process_single_entity_historical_score() — single entity with retry
    - process_historical_score_bulk_extraction_parallel() — bulk parallel processing
"""
...
from tests.conftest import HISTORICAL_SCORE_WKT

pytestmark = pytest.mark.public

# ===================================================================
# process_single_entity_historical_score()
# ===================================================================


class TestProcessSingleEntityHistoricalScore:
    @patch(
    ...
    def test_successful_entity_returns_dataframe(
        ...
        mock_wkt,
        mock_normalize,
        configured_historical_score_extractor,
        sample_historical_score_entity,
        sample_historical_score_response,
    ):
        mock_retry.return_value = sample_historical_score_response

        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            result = configured_historical_score_extractor.process_single_entity_historical_score(
                sample_historical_score_entity
            )

        assert result["error"] is None
        assert isinstance(result["data"], pd.DataFrame)
        assert result["data"].iloc[0]["risk_score"] == 0.22
    ...
    def test_empty_data_returns_error(
        self, mock_retry, mock_wkt, configured_historical_score_extractor, sample_historical_score_entity
    ):
        """Response with no 'data' key → 'No historical_score results found'."""
        ...
        assert "No historical_score results found" in result["error"]["message"]
    ...
    def test_validation_failure_returns_error(
        self, mock_wkt, configured_historical_score_extractor, sample_historical_score_entity
    ):
        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            result = configured_historical_score_extractor.process_single_entity_historical_score(
                sample_historical_score_entity
    ...
    def test_invalid_crop_returns_error(self, configured_historical_score_extractor):
        entity = {"id": "ent_bad", "geometry": HISTORICAL_SCORE_WKT, "crop": "BANANA"}

        with patch("earthdaily.agriculture.processors.processor_score_functions.validate_wkt", side_effect=lambda x: x):
            with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
                result = configured_historical_score_extractor.process_single_entity_historical_score(entity)
    ...
    def test_retry_called_with_correct_params(
        ...
        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            configured_historical_score_extractor.process_single_entity_historical_score(sample_historical_score_entity)
    ...
    def test_pandas_series_with_historical_seasons(
        ...
        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            result = configured_historical_score_extractor.process_single_entity_historical_score(row)
...
class TestBulkExtractionParallel:
    ...
    @patch.object(HistoricalScoreExtractor, "process_single_entity_historical_score")
    @patch.object(HistoricalScoreExtractor, "_finalize_extraction", return_value=_FINALIZE_RETURN)
    @patch.object(HistoricalScoreExtractor, "_merge_with_skipped_entities")
    def test_bulk_all_success(
        ...
        mock_export,
        configured_historical_score_extractor,
        sample_historical_score_entity_list,
    ):
        success_df = pd.DataFrame([{"entity_id": "ent", "average_potential_score": 0.78, "risk_score": 0.22}])
        mock_single.return_value = {"data": success_df, "error": None}
        mock_merge.side_effect = lambda **kw: kw["new_results_df"]

        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            result = configured_historical_score_extractor.process_historical_score_bulk_extraction_parallel(
                entity_list=sample_historical_score_entity_list,
                max_workers=2,
                skip_export=True,
            )
    ...
    def test_bulk_mixed_results(
        ...
        def side_effect(*args, **kwargs):
            ...
            if call_count["n"] <= 2:
                return {
                    "data": pd.DataFrame([{"entity_id": "x", "risk_score": 0.2}]),
                    "error": None,
                }
    ...
    def test_bulk_invalid_merge_existing_raises(
        self, configured_historical_score_extractor, sample_historical_score_entity_list
    ):
        with patch.object(configured_historical_score_extractor, "ensure_token_valid"):
            with pytest.raises(ValueError, match="merge_existing"):
                configured_historical_score_extractor.process_historical_score_bulk_extraction_parallel(
                    entity_list=sample_historical_score_entity_list,
    ...
    def test_bulk_filter_exclude(
        ...
        """Notebook cell 27 uses filter_type='exclude' on crop=CORN."""
        entity_list = pd.DataFrame(
            [
                {"id": "ent_001", "geometry": HISTORICAL_SCORE_WKT, "crop": "OTHERS"},
                {"id": "ent_002", "geometry": HISTORICAL_SCORE_WKT, "crop": "CORN"},
                {"id": "ent_003", "geometry": HISTORICAL_SCORE_WKT, "crop": "CORN"},
            ]
        )
